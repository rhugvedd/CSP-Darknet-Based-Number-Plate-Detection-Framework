...
class YoloLoss(nn.Module):
    ...
    def forward(self, predictions, targets, anchor_nos):
        ...
        object_present = torch.zeros(m, w, h, anchor_nos, dtype = torch.int8)
        no_object = torch.ones(m, w, h, anchor_nos, dtype = torch.int8)
        ...
        loss += torch.sum(object_present * ((C_hat * torch.log(C)) + ((1 - C_hat) * torch.log(1 - C))))

        loss += self.lambda_no_obj * torch.sum(no_object * ((C_hat * torch.log(C)) + ((1 - C_hat) * torch.log(1 - C))))

        loss /= targets.size[0]

        return loss

    def CIoU_Loss(self, pred_boxes, gt_boxes):
        
        bbox_coords = lambda box: (box[..., 0] - (box[..., 2] / 2), 
                                   box[..., 1] - (box[..., 3] / 2),
                                   box[..., 0] + (box[..., 2] / 2), 
                                   box[..., 1] + (box[..., 3] / 2))
        ...
        union_area = (pred_box[..., 2] * pred_box[..., 3]) + (gt_box[..., 2] * gt_box[..., 3]) - inter_area
        ...
        dist_loss = sq_dist / sq_diag
        
        # Aspect Ratio Loss -----------------------------------------------------------------------------------------
        
        v = 0.40528 *   (
                            (
                                torch.atan(gt_boxes[..., 2] / gt_boxes[..., 3])
                                -
